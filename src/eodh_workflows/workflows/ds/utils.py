...
from eodh_workflows.consts.stac import (
...
from eodh_workflows.utils.raster import save_cog_v2
from eodh_workflows.utils.stac import prepare_stac_asset
...
def download_search_results(
    ...
    *,
    ...
    for item in progress_bar:
        ...
        with (
            LocalCluster(n_workers=N_WORKERS, threads_per_worker=THREADS_PER_WORKER) as cluster,
            DistributedClient(cluster),
        ):
...
def split_s2_ard_cogs_into_separate_assets(fps: Iterable[Path]) -> None:
    for fp in fps:
        ...
        if "cog" not in item.assets:
            _logger.warning("No COG asset found in %s. Skipping...", fp.name)
            continue
